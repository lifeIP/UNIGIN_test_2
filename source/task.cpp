	...
	auto check_the_visibility_fields = [&](const std::vector<unit>& data, const std::vector<int>& input_ID, std::vector<int>& output, const int id) {
		...
		for (int i = id - 1; i >= 0; --i) {
			...
			if (dist > data.at(input_ID.at(id)).distance) {
				if (data.at(input_ID.at(id)).position.x + 100000 + data.at(input_ID.at(id)).position.y + 100000 + data.at(input_ID.at(id)).distance <
					data.at(input_ID.at(i)).position.x + 100000 + data.at(input_ID.at(i)).position.y + 100000) break;
				continue;
			}
